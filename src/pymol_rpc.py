...
class pymol_jupyter_builder:
    ...
    def __init__(self, host=HOST, port=PORT, exe="pymol"):
        self.host = host
        self.port = int(port)
        self._process = None
    ...
    def Get_Object_Names(self):
        curr_path = os.getcwd()
        with open(curr_path+'/Get_Object_Names.py','w') as input:
            input.write(string_get_object_name)
        self.server.do('run Get_Object_Names.py')
        time. sleep(2)
        #print(os.path.exists(curr_path+'/object_name_mw_dbe.txt'))
        with open(curr_path+'/object_name_mw_dbe.txt','r') as output:
    ...
    def Crosslink_mols(self,object1,index1,object2,index2):
        ...
        object_names = self.Get_Object_Names().keys()
    ...
    def Attach_CH3(self,object1,h_index):
        ...
        self.server.do('load ./functional_groups/Methane.pdb')
    ...
    def Attach_CH2CH3(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Ethane.pdb')
    ...
    def Attach_OH(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Water.pdb')
    ...
    def Attach_SH(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Sulfide.pdb')
    ...
    def Attach_Aldehyde(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Aldehyde.pdb')
    ...
    def Attach_Carboxyl(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Carboxylic.pdb')
        self.server.do('select at2, %s & name %s' % ('Carboxylic', 'H1'))
        self.server.do('edit at2, at1')
        self.server.do('fuse')
        self.server.do('unpick')
        self.server.do('delete at1')
        self.server.do('delete at2')
        self.server.do('delete %s' % 'Carboxylic')
        self.server.do('rebuild all')
        self.server.do('zoom %s' % object1)
        self.server.do('clean %s' % object1)
        self.refresh_model()
    ...
    def Attach_Ester(self,object1,h_index):
        self.server.do('select at1, %s & index %s' % (object1, h_index))
        self.server.do('load ./functional_groups/Ester.pdb')
        self.server.do('select at2, %s & name %s' % ('Ester', 'H1'))
        self.server.do('edit at2, at1')
        self.server.do('fuse')
        self.server.do('unpick')
        self.server.do('delete at1')
        self.server.do('delete at2')
        self.server.do('delete %s' % 'Ester')
        self.server.do('rebuild all')
        self.server.do('zoom %s' % object1)
        self.server.do('clean %s' % object1)
        self.refresh_model()
    ...
    def examine_cnmr(self,object,color=False):
        ...
        if color: 
            self.server.do('color red,   elem o')
            self.server.do('color grey,  elem c')
            self.server.do('color purple, elem h')
            self.server.do('color blue,  elem n')
            self.server.do('color pink,  elem si')
    ...
    def color_idx(self,object,idx,color_name):
        #self.refresh_model()
        for id in idx:
            self.server.do('color %s, %s and index %d' % (color_name,object,id))
