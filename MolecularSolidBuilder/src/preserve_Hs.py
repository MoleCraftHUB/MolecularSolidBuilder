...
pdb_filename = '1-s2.0-S0016236111007940-mmc2.pdb'
...
for ai, m in enumerate(ms):
	...
	#m = AllChem.RemoveHs(m, implicitOnly=False)
	atoms = m.GetAtoms()
	#print([[atom.GetSymbol(), atom.GetTotalNumHs(), atom.GetNumExplicitHs(), atom.GetNumImplicitHs()] for atom in atoms])
	#print([[atom.GetSymbol(), atom.GetTotalNumHs(), [n.GetSymbol() for n in atom.GetNeighbors()] ] for atom in atoms])

	for atom in atoms:
		if atom.GetSymbol() == 'C':
			if len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() != 'H'] )== 2 and \
			   len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() == 'H'] )== 1:
				atom.SetNumRadicalElectrons(1)
			if len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() != 'H'] )== 3 and \
			   len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() == 'H'] )== 0:
				atom.SetNumRadicalElectrons(1)
		...
		if atom.GetSymbol() == 'N':
			if len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() != 'H'] )== 2 and \
			   len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() == 'H'] )== 0:
				atom.SetNumRadicalElectrons(1)
		if atom.GetSymbol() == 'O':
			if len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() != 'H'] )== 1 and \
			   len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() == 'H'] )== 0:
				atom.SetNumRadicalElectrons(1)
		if atom.GetSymbol() == 'S':
			if len( [n.GetSymbol() for n in atom.GetNeighbors() if n.GetSymbol() != 'H'] )== 1 and \
	...
	#print([[atom.GetNumRadicalElectrons(), atom.GetHybridization()] for atom in atoms if atom.GetNumRadicalElectrons() == 1])
	[atom.SetHybridization(Chem.rdchem.HybridizationType.SP2) for atom in atoms if atom.GetNumRadicalElectrons() == 1]
	...
	for ra, rb in zip(ring_members, ring_bonds):
		test = [True if atoms[r].GetHybridization() == Chem.rdchem.HybridizationType.SP2 else False for r in ra]
		if all(test):
			[atoms[r].SetIsAromatic(True) for r in ra]
			[bonds[r].SetBondType(Chem.rdchem.BondType.AROMATIC) for r in rb]
			[atoms[r].SetNumRadicalElectrons(0) for r in ra]
	
	for bond in bonds:
		a = bond.GetBeginAtom()
		b = bond.GetEndAtom()
		if a.GetNumRadicalElectrons() == 1 and b.GetNumRadicalElectrons() == 1:
	...
	#Draw.MolToFile(m,'./png/test_%d.png' % ai,size=(1000,1000),highlightAtoms=highlight_atoms,highlightBonds=highlight_bonds)
	Draw.MolToFile(m,'./png/test_%d.png' % ai,size=(1000,1000))
	subprocess.call('imgcat ./png/test_%d.png' % ai,shell=True)
